<x-app-layout>
    <div x-data="{ 
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex items-center justify-between gap-4 mb-8">
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 md:w-16 md:h-16 bg-slate-900 rounded-2xl md:rounded-[1.8rem] flex items-center justify-center text-white shadow-xl shadow-slate-200 shrink-0">
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="flex items-center gap-2 mt-2">
                                <span class="text-[9px] md:text-[11px] font-bold text-slate-400 uppercase tracking-widest">{{ strtoupper(\Carbon\Carbon::now()->translatedFormat('l, d F Y')) }}</span>
                                <span class="w-1 h-1 bg-slate-200 rounded-full"></span>
                                <span class="text-[9px] md:text-[11px] font-black text-emerald-500 uppercase tracking-widest">PERIODE {{ $classroom->current_period }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2 md:gap-3 shrink-0">
                        <!-- Next Period Action -->
                        <button @@click="showNextPeriodModal = true" class="flex items-center justify-center gap-2.5 px-4 h-10 md:h-14 md:px-6 bg-emerald-600 text-white rounded-2xl md:rounded-[1.5rem] shadow-lg shadow-emerald-100 hover:bg-emerald-700 active:scale-95 transition-all group overflow-hidden relative">
                        ...
                        <button @@click="showDrawer = true" class="flex items-center justify-center gap-2.5 px-3 h-10 md:h-14 md:px-6 bg-slate-900 text-white rounded-2xl md:rounded-[1.5rem] shadow-xl shadow-slate-200 hover:bg-slate-800 active:scale-95 transition-all">
                        ...
                        <div x-data="{ menuOpen: false }" class="relative">
                            <button @@click="menuOpen = !menuOpen" @@click.away="menuOpen = false" class="w-10 h-10 md:w-14 md:h-14 bg-white border border-slate-100 rounded-2xl md:rounded-[1.5rem] flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-50 transition-all shadow-sm">
                            ...
                            <div x-show="menuOpen" x-transition.origin.top.right class="absolute right-0 mt-3 w-48 bg-white border border-slate-100 rounded-2xl md:rounded-[1.5rem] shadow-2xl py-2 z-50" style="display: none;">
                ...
                <div class="mb-10 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <!-- Saldo Card -->
                        <div class="md:col-span-2 relative h-40 md:h-auto rounded-[2.5rem] md:rounded-[3rem] overflow-hidden shadow-2xl shadow-emerald-100/50 flex flex-col justify-center items-center text-center p-8">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-1 gap-3 md:gap-4 h-full">
                            <div class="bg-white rounded-[2rem] md:rounded-[2.5rem] p-4 md:p-6 border border-slate-100 shadow-sm flex items-center md:justify-center gap-4 group transition-all hover:border-emerald-100">
                            ...
                            <div class="bg-white rounded-[2rem] md:rounded-[2.5rem] p-4 md:p-6 border border-slate-100 shadow-sm flex items-center md:justify-center gap-4 group transition-all hover:border-rose-100">
                    ...
                    <!-- Member Count Card -->
                    <div class="bg-white rounded-[2rem] md:rounded-[3rem] p-6 border border-slate-100 shadow-sm flex items-center md:justify-center gap-4 group hover:border-emerald-100 transition-all">
                        <div class="w-10 h-10 md:w-14 md:h-14 bg-slate-50 text-slate-300 rounded-2xl md:rounded-full group-hover:bg-emerald-50 group-hover:text-emerald-500 transition-colors flex items-center justify-center shrink-0 overflow-hidden relative">
                ...
                <!-- Activity List -->
                <div class="bg-white rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-2xl shadow-slate-200/40 overflow-hidden mb-12">
                ...
                <div class="mb-8">
                    ...
                    <div x-show="totalPages > 1" class="mt-10 flex items-center justify-center gap-2">
                        <button @@click="currentPage--" :disabled="currentPage === 1" class="w-10 h-10 rounded-xl bg-white border border-slate-100 flex items-center justify-center text-slate-400 disabled:opacity-30">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M15 19l-7-7 7-7"/></svg>
                        </button>
                        <div class="px-5 h-10 bg-white border border-slate-100 rounded-xl flex items-center justify-center">
                            <span class="text-[10px] font-black text-slate-800 uppercase tracking-widest">Page <span x-text="currentPage"></span> of <span x-text="totalPages"></span></span>
                        </div>
                        <button @@click="currentPage++" :disabled="currentPage === totalPages" class="w-10 h-10 rounded-xl bg-white border border-slate-100 flex items-center justify-center text-slate-400 disabled:opacity-30">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select name="type" x-data="{ txType: 'income' }" x-model="txType" class="w-full border-none rounded-2xl font-black text-[10px] md:text-xs p-4 py-[1.125rem] focus:ring-0 transition-all uppercase tracking-widest shadow-inner cursor-pointer" :class="txType === 'expense' ? 'bg-rose-50 text-rose-600' : 'bg-emerald-50 text-emerald-600'">
        ...
        <div x-show="showConfirmModal" class="fixed inset-0 z-[200] flex items-center justify-center p-6" style="display: none;" x-transition>
            ...
            <div x-show="showConfirmModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="relative w-full max-w-sm bg-white rounded-[2.5rem] md:rounded-[3rem] shadow-3xl overflow-hidden p-8 md:p-10 text-center">
        ...
        <div x-show="showNextPeriodModal" class="fixed inset-0 z-[200] flex items-center justify-center p-6" style="display: none;" x-transition>
            ...
            <div x-show="showNextPeriodModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="relative w-full max-w-sm bg-white rounded-[2.5rem] md:rounded-[3rem] shadow-3xl overflow-hidden p-8 md:p-12 text-center">
        ...
        <div x-show="showMemberDrawer" class="fixed inset-0 z-[120] overflow-hidden" style="display: none;">
            ...
            <div class="fixed inset-y-0 right-0 max-w-full flex pl-10">
                <div x-show="showMemberDrawer" x-transition:enter="transform transition ease-in-out duration-500" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0" class="w-screen max-w-md">
                    <div class="h-full flex flex-col bg-white shadow-3xl rounded-l-[2.5rem] md:rounded-l-[3.5rem] overflow-hidden" x-data="{ gender: 'male' }">
        ...
        <div x-show="showEditDrawer" class="fixed inset-0 z-[120] overflow-hidden" style="display: none;">
            ...
            <div class="fixed inset-y-0 right-0 max-w-full flex pl-10">
                <div x-show="showEditDrawer" x-transition:enter="transform transition ease-in-out duration-500" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0" class="w-screen max-w-md">
                    <div class="h-full flex flex-col bg-white shadow-3xl rounded-l-[2.5rem] md:rounded-l-[3.5rem] overflow-hidden">
                        ...
                        <form action="{{ route('classrooms.update', $classroom) }}" method="POST" class="flex-1 p-6 md:p-10 space-y-8 overflow-y-auto custom-scrollbar">
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="p-6 bg-emerald-50 rounded-[2.5rem] border border-emerald-100 shadow-sm">
                    ...
                    <template x-if="activeMember">
                        <div class="space-y-10">
                            ...
                            <template x-if="activeMember.isUnpaid">
                                <div class="bg-rose-50 rounded-[2.5rem] p-6 md:p-8 border border-rose-100 shadow-xl shadow-rose-100/50">
