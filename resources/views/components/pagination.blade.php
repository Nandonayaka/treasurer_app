...
@if($alpine)
    <div x-show="{{ $total }} > 1" {{ $attributes->merge(['class' => 'flex items-center justify-center gap-2 md:gap-3']) }}>
        <button @click="{{ $current }}--" :disabled="{{ $current }} === 1" 
            class="p-2.5 md:p-3 bg-white border border-slate-100 rounded-xl md:rounded-2xl text-slate-400 disabled:opacity-30 disabled:cursor-not-allowed hover:text-emerald-600 hover:border-emerald-100 transition-all shadow-sm active:scale-95">
        ...
        <div class="flex items-center gap-2 px-4 md:px-6 py-2 md:py-3 bg-white border border-slate-100 rounded-xl md:rounded-2xl shadow-sm">
            <span class="text-[10px] md:text-sm font-black text-emerald-600" x-text="{{ $current }}"></span>
            <span class="text-[8px] md:text-[10px] font-bold text-slate-300">/</span>
            <span class="text-[10px] md:text-sm font-black text-slate-800" x-text="{{ $total }}"></span>
        </div>

        <button @click="{{ $current }}++" :disabled="{{ $current }} === {{ $total }}" 
            class="p-2.5 md:p-3 bg-white border border-slate-100 rounded-xl md:rounded-2xl text-slate-400 disabled:opacity-30 disabled:cursor-not-allowed hover:text-emerald-600 hover:border-emerald-100 transition-all shadow-sm active:scale-95">
...
@elseif($paginator && $paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" {{ $attributes->merge(['class' => 'flex items-center justify-center gap-2 md:gap-3']) }}>
        {{-- Previous Page Link --}}
